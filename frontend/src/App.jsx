...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetchTransactions(selectedUser, controller.signal)
      ...
      .catch((error) => {
        if (error.name !== "AbortError") setTransactionsError(error.message);
      })
      .finally(() => {
    ...
    fetchSubscriptions(selectedUser, controller.signal)
      ...
      .catch((error) => {
        if (error.name !== "AbortError") setSubscriptionsError(error.message);
      })
      .finally(() => {
  ...
  const inspect = () => {
    // Diagnostic results are fetched separately from finalized subscriptions so
    // opening the inspector cannot replace the dashboard's persisted decisions.
    ...
    fetchMerchantGroups(selectedUser, controller.signal)
      .then(setGroups)
      .catch((error) => {
        if (error.name !== "AbortError") setGroupsError(error.message);
      })
      .finally(() => {
